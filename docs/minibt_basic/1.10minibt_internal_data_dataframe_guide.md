...
# 创建空 IndFrame（不带任何参数），后续动态添加列
df = IndFrame()                      # 空指标容器
df.close = range(100)                # 属性方式添加列
df["open"] = range(100)              # 键方式添加列
```
...
# 1. 创建空 IndFrame，通过属性赋值逐列添加指标
df = IndFrame()
df.close = range(100)
df.ma5 = df.close.sma(5)
df.ma20 = df.close.sma(20)
df.signal = df.ma5.cross_up(df.ma20)
print(df.head())      # 已包含 close、ma5、ma20、signal 四列
df.btplot()           # 可正常绘图

# 2. 基于 KLine 动态构建多信号指标容器
...
ind.ma_fast = ma_fast
ind.ma_slow = ma_slow
ind.long_signal = ma_fast.cross_up(ma_slow)
ind.short_signal = ma_fast.cross_down(ma_slow)

# 指标自动关联 kline 数据源，btplot 正常显示
...
# 3. 混合使用属性赋值和键赋值
df = IndFrame()
df.close = range(100)      # 属性方式
df["open"] = range(100)    # 键方式
df["ma10"] = df.close.sma(10).to_numpy()
```
