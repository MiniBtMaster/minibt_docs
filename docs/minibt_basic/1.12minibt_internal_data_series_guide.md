...
### 4. 数据兼容性强
- 支持多种输入数据类型（Series、ndarray、整数、None）
- `IndSeries()` 不带参数创建空序列，后续通过 `[:]` 赋值填充
...
# 创建空 IndSeries（v1.2.7+，不带任何参数），后续通过 [:] 赋值填充
s = IndSeries()     # len=0 的空白定义指标序列
s[:] = range(10)    # 整体赋值，自动扩容到长度为10
```
...
### 自定义指标生成
```python
# 方式一：空序列 + [:] 整体赋值（v1.2.7+ 推荐）
custom_series = IndSeries()
custom_series[:] = np.random.randn(100)  # 自动扩容并填充数据
...
# 方式三：从表达式计算结果创建
s = IndSeries()
s[:] = original_data * 0.7 + other_data * 0.3
```
...
4. **强大的扩展能力**：支持多种数据输入格式（含 `IndSeries()` 空初始化 + `[:]` 动态赋值），提供灵活的数据转换方法
5. **无缝第三方集成**：直接访问 PandasTA、TA-Lib 等流行指标库
6. **优雅的空序列构建**：`IndSeries()` 创建空自定义序列，`series[:] = data` 整体赋值，自动适配长度扩容
