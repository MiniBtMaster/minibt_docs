...
✅ **策略类 `op_config` 自动检测优化配置**

- 策略类上定义 `op_config = OpConfig(...)` 后，`Bt.run()` 自动识别并切换为参数优化模式
- 无需手动调用 `bt.optstrategy()`，减少重复代码
- 兼容原有 `bt.optstrategy()` 手动调用方式

### 修复

- **多策略 KLine 子行 ID 同步修复**：多策略场景下，KLine 对象的 `id.strategy_id` 更新后，其内部 Line 对象（`close`、`low`、`high`、`open`、`volume` 等）的 `id` 未同步更新，导致 Stop 止损止盈读取到错误策略的 K 线数据。修复为遍历 `KLine.lines` 同步更新所有子行的 `id`
- **参数优化前清空全局策略实例**：`Bt.run()` 优化分支添加 `Base._strategy_instances` 清空逻辑，避免上次回测残留的全局策略实例干扰优化结果
- **light_chart / light_chart_replay PyQt6 兼容性全面修复**：GUI 界面从 PySide6-Fluent-Widgets 迁移至 PyQt6-Fluent-Widgets，解决相关兼容性问题。

## [1.2.4] - 2026-05-31

### 新增

✅ **`Bt.run()` 支持 `gui="light_chart"` 参数**

- 回测完成后可使用 Lightweight Charts（Qt 界面）展示 K 线、指标与交易信号，作为传统 Bokeh 图表的替代方案

✅ **light_chart 回测结果图表展示模式**：

- `Bt(replay=False).run(isplot=True, gui="light_chart")` 一次性加载全部回测数据，隐藏回放控制栏，用于快速浏览回测结果

✅ **light_chart K线回放模式**：

- `Bt(replay=True).run(gui="light_chart")` 逐根 K 线推进回放，支持暂停/继续、速度调节


✅ **实盘运行模式重构——支持三种运行模式**：

- **图表模式** `bt.run(isplot=True)`：启动 Qt GUI + lightweight-charts，完整可视化
- **后台串行模式** `bt.run(isplot=False)`：单进程多策略，无图表、共享一个 TqApi 连接
- **并行模式** `bt.run(isplot=False, run_parallel=True)`：每个策略独立进程，无图表，独立 TqApi 连接

✅ **并行模式核心实现**：

- 新增 `__tq_real_parallel()` 方法，通过 `multiprocessing.Process` 为每个策略 fork 独立子进程
- 每个子进程独立创建 `Bt(live=True)` 和 `TqApi`，避免共享连接导致的资源冲突
- 父进程负责 fork 和统一等待，`Ctrl+C` 可优雅终止所有子进程
- 任一策略崩溃不影响其他策略运行

✅ **后台串行模式**：

- 新增 `__tq_real_headless()` 方法，纯后台循环 `wait_update` → `_execute_live_trading()`
- 不启动 `MainWindow`、`LightChartWindow`，无 Qt/GUI 依赖，适合服务器部署

✅ **认证信息持久化**：

- `addTqapi()` 内部保存 `tq_auth` 和 `tq_account` 参数值
- 并行模式下子进程复用认证信息独立创建 `TqApi`

✅ **Bt 类新增 `_load_strategies` 方法**：

- 自动从 `.py` 文件或文件夹中发现并加载所有 `Strategy` 子类
- 支持三种传参方式：模块名（`"cci"`）、文件路径（`"cci.py"`）、文件夹路径（`"."`）
- 通过 `issubclass(obj, Strategy)` 自动识别策略类，智能排除非策略类
- 支持 `importlib.import_module` 标准导入，失败时回退到 `spec_from_file_location` 独立文件导入

✅ **`addstrategy` 方法支持 `str` 路径参数**：

- `bt.addstrategy(".")` 一行即可自动发现并加载目录下所有策略文件
- 字符串路径时自动调用 `_load_strategies` 发现 Strategy 子类
- 与原有 `Strategy` 类参数混用兼容，支持 `bt.addstrategy(MyStrategy, ".")`

✅ **`IndSeries()` 支持空初始化**：

- `IndSeries()` 不带参数创建空指标序列（`len=0`），后续可通过 `series[:] = data` 整体赋值填充数据

✅ **`IndFrame()` 支持空初始化**：

- `IndFrame()` 不带参数创建空指标容器，后续可通过属性赋值（`df.col = data`）或键赋值（`df["col"] = data`）动态添加指标列

✅ **`OptimizeConfig` dataclass**：

- 统一管理 `signal_backtest` 参数优化配置，支持网格搜索与 Optuna 贝叶斯优化的单目标/多目标/权重配置


### 修复

- **`IndicatorsBase.__setitem__` 修复 `[:]` 全切片扩容赋值**：修复空序列 `series[:] = data` 赋值后 `pandas_object` / `copy_object` 仍为空的问题（`copy()` 方法循环依赖导致），现改为直接从底层 numpy 数据构造新的 pandas 对象
- **`IndFrame.__setitem__` 修复 `dtype` 属性错误**：`IndFrame` 没有 `dtype` 属性，改为 `getattr(self, 'dtype', None)` 兜底，避免 `IndFrame` 直接添加列时报错
- **`btplot` 修复死循环**：当指标没有关联 KLine 数据源时，追溯 `source_object` 的 `while` 循环条件 `cond or len < 100` 导致无限循环，改为 `cond and len < 100` 并增加 `None` 检查与异常 `break`；无 KLine 时自动退回到 pandas 原生 `plot()` 方法

### 文档

- **[实时图表系统Lightweight Charts介绍](minibt_basic/1.19minibt_lightweight_charts.md)**：补充 `isplot=False` 后台模式说明
- **[三种实盘运行模式介绍](minibt_basic/1.21minibt_realtime_trading_modes.md)**：新增文档，详细介绍图表/串行/并行三种模式的调用方式、适用场景与完整示例
- **[signal_backtest 止损止盈参数全量测试案例](minibt_basic/1.22minibt_signal_backtest_stop_profit_test.md)**：新增文档，覆盖 sl_stop / tp_stop / sl_trail / sl_callback / tp_callback / max_hold_bars 等全部止损止盈参数的 20 项组合测试
- **[signal_backtest 指标信号参数优化详解](minibt_basic/1.23minibt_signal_backtest_optimize.md)**：新增文档，覆盖 `OptimizeConfig` 用法、`test_optimize.py` 完整案例及 11 种优化场景分析
- **[IndFrame 完整指南](minibt_basic/1.10minibt_internal_data_dataframe_guide.md)**：新增空初始化示例（从 `indframe_assign_build.ipynb` 摘录）
- **[IndSeries 完整指南](minibt_basic/1.12minibt_internal_data_series_guide.md)**：新增 `IndSeries()` 空初始化与 `[:]` 赋值示例
- **[策略回放与回测结果图表展示](minibt_basic/1.24minibt_replay_and_light_chart_display.md)**：新增文档，介绍 light_chart 回放模式与回测结果展示模式的用法、参数及与 Bokeh 的对比

✅ **分组窗口计算能力完善（对pandas升级后完善的功能）**：

- 完善 `BtExponentialMovingWindowGroupby` 类，实现分组指数加权移动窗口计算：

    - 继承自 `BtExponentialMovingWindow`，每个分组独立维护 EWM 状态，分组间互不干扰
    - 内部通过 `BaseGrouper` 管理分组索引，构造 `ExponentialMovingWindowGroupby` 对象
    - 支持 `mean`、`std`、`var`、`corr`、`cov`、`aggregate` 等全部父类方法
    - 支持 `.online()` 获取在线增量计算模式
- `IndicatorsBase` 新增 `ewm_groupby(by, ...)` 方法：

    - 对应 pandas `groupby(...).ewm()`，返回 `BtExponentialMovingWindowGroupby` 对象
    - 支持 `span`、`com`、`halflife`、`alpha` 等衰减参数，`selection` 列选择
    - 适用于分组 EMA、分组指数加权波动率等场景
- `IndicatorsBase` 新增 `expanding_groupby(by, ...)` 方法：

    - 对应 pandas `groupby(...).expanding()`，返回 `BtExpandingGroupby` 对象
    - 与现有 `expanding()` 形成对称设计，填补 "有类无入口" 的空缺
    - 适用于分组累积统计（每个分组独立的历史最高价、累积均值等）
...
- 框架现已支持 Python 3.12 及以上版本
- 修复了 Pandas 3.0+ 严格类型匹配要求带来的兼容性问题
- 添加 Copy-on-Write 兼容性处理

### 新增

✅ **indicators 子包重构**：

- 将 indicators.py 拆分为多个子模块，提高代码可维护性
- 子包结构：`__init__.py`、`_typing.py`、`_base.py`、`_core.py`、`_pandas_ta.py`、`_btind.py`、`_tulip.py`、`_tqfunc.py`、`_tqta.py`、`_talib.py`、`_finta.py`、`_pair.py`、`_factors.py`
- 添加 `MinibtILocIndexer` 和 `MinibtLocIndexer` 索引器类
- 新增 `__getitem__` 和 `__setitem__` 方法，支持 `[]` 操作符
- 索引器自动将返回的 Pandas 对象转换为框架内置指标类型
- 在 `__init__.py` 中新增 `Line` 类导出

✅ **新增基于信号的回测函数bt_from_signals**：

- 新增Cython实现的高性能回测函数，支持多合约并行回测
- 支持灵活的参数配置，包括交易大小、保证金率、价格跳动、成交量乘数等
- 支持不同合约设置不同的参数值，实现精细化回测
- 集成到minibt策略框架中，可通过策略实例直接调用
...
- 为bt_from_signals函数添加详细的日志记录功能
- 记录回测参数、执行过程和结果统计信息
- 提供每个合约的详细回测结果，包括最终权益、累计盈亏和总手续费

✅ **易用性改进**：

- 提供简洁的API接口，与minibt策略的使用方式保持一致
- 支持IndSeries对象作为输入信号，与现有指标系统无缝集成
- 自动处理数据维度，确保输入数据格式正确
...
- 修复了装饰器中获取底层 DataFrame 的逻辑，从检查 `_df` 属性改为优先检查 `pandas_object` 属性
- 解决了 `obv`、`cdl_inside` 等不带 `pta_` 前缀的 pandas_ta 指标调用失败的问题
- 提高了指标计算的健壮性和兼容性

✅ **循环导入问题修复**：

- 使用延迟导入（lazy import）解决模块间的循环依赖
- 在 `_base.py` 中对 `IndFrame`、`IndSeries`、`Line`、`CoreIndicators` 等类使用延迟导入
- 在 `_core.py` 索引器类中实现智能类型转换逻辑
- 确保切片操作（如 `iloc[:50]`、`[:1000]`）后仍返回正确的框架类型

✅ **交易执行逻辑优化**：

- 修改Broker.create_order方法，当exectype == OrderType.Close时立即执行交易，无需等待update_history函数处理
- 确保BtAccount.update_history函数中跳过已即时处理的订单

✅ **交易日志准确性提升**：

- 修复交易日志中操作类型的问题，确保开空对应平空，开多对应平多
- 优化Broker.update函数中的操作类型判断逻辑，使用current_position代替long参数

✅ **成本价计算优化**：

- 改进成本价(cost_price)的手续费计算逻辑，考虑开仓方向的影响
- 多仓时手续费为正，空仓时手续费为负，确保成本价计算准确

✅ **回测一致性提升**：

- 确保backtrader_from_signals.pyx与minibt策略的累计盈亏计算一致
- 优化累计收益计算逻辑，保持与minibt策略的计算方式同步

✅ **性能优化**：

- 使用Cython编译提高回测速度，处理大规模数据时性能显著提升
- 优化内存使用，减少不必要的计算和数据复制
- 支持并行处理多个合约，提高回测效率
...
✅ **全局配置新增 set_data_patching 开关**：

- 默认值：False（优先保证回测速度）
- False 状态：调用指标/K线数据返回全量数据，回测性能最优
- True 状态：next函数中自动替换最新数据，调用指标/K线仅返回最新数据（回测速度会下降）
- 配置归属：MinibtOptions 全局配置类，通过`options.set_data_patching = True`开启或关闭
- 开启状态时在`next`函数中所有指标为最新数据，非开启状态时在`next`函数中所有指标为全量数据
  
### 优化

✅ **实时图表升级为PySide6**：

- 实时图表从PyQt5迁移至Fluent Design风格组件库PySide6-Fluent-Widgets
- 优化了柱状体在0轴上下显示的颜色
...
- OrderType.Close 订单类型，指定在当前 K 线收盘价成交；
- OrderType.Market 订单类型，指定在下一根 K 线开盘价成交；
- 限价单 (Limit)、止损单 (Stop) 等其余订单类型执行逻辑保持不变。

✅ **优化停止类(Stop)的使用流程**：

- **初始化扩展**：`__init__`方法支持任意指标的前置计算，为停止策略提供灵活的预处理能力
- **KLine全访问**：无限制调用KLine对象的所有属性/方法，满足各类技术分析需求
- **标准化接口**：通过`__init__/long/short`三板斧即可实现自定义停止策略，降低开发成本
- **多头精细化**：`long()`方法专管多头停止价/目标价，逻辑边界清晰，便于维护和扩展
- **空头精细化**：`short()`方法专管空头停止价/目标价，与多头逻辑对称，适配完整的多空交易场景
- **[停止类介绍](minibt_basic/1.20minibt_stop.md)**
...
- **[实时图表介绍](minibt_basic/1.19minibt_lightweight_charts.md)**
...
- **[新增订单系统详细介绍](minibt_basic/1.18minibt_order_system.md)**：完整的订单系统架构和使用指南
...
- **新增内置指标绘图函数 `btplot`**
    ...
    - 查看 [`btplot` 函数功能说明](minibt_basic/1.17minibt_interactive_financial_charting_with_btplot.md)
...
- **基于 Rich 的专业交易日志库**
    ...
    - 表情符号辅助识别，提升日志可读性
    - 查看 [日志简介](minibt_basic/1.16minibt_transaction_log.md)
...
---
[1.2.6]: https://pypi.org/project/minibt/1.2.6/
[1.2.5]: https://pypi.org/project/minibt/1.2.5/
[1.2.3]: https://pypi.org/project/minibt/1.2.3/
[1.2.2]: https://pypi.org/project/minibt/1.2.2/
[1.2.1]: https://pypi.org/project/minibt/1.2.1/
